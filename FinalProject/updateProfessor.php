...
///////////////////////////////////////////////////////////////////////////////////////////
		...
		$query = "UPDATE Professor SET FName=?,LName=?,Email=? WHERE ProfessorID=?";

		//Prepare and execute query (use $_POST values from submitted form)
		$stmt = $mysqli->prepare($query);
		$stmt->execute([$_POST["FName"], $_POST["LName"], $_POST["Email"], $_POST["ProfessorID"]]);
	  ...
	  $stmt->execute([$ID]);
		...
		if ($stmt)  {
			...
			echo "<h3>".$row["FName"]." ".$row["LName"]."'s Information</h3>";
      		echo "<form action='updateProfessor.php' method='POST'>";
			echo "<p><input type='hidden' name='ProfessorID' value='{$ID}'> </p>";
...
///////////////////////////////////////////////////////////////////////////////////////////
		...
		//Query failed. Return to readS21.php and output error
			...
			$_SESSION["message"] = "Professor could not be found!";
			redirect("home.php");
		}
	  }
    }
